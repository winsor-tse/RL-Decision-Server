...
- `Inference/ppo_lstm_eval.py`: evaluates recurrent PPO checkpoints on `Mage`.
...
- `Inference/any_percent_bc_eval.py`: evaluates BC checkpoints on Minari data or `Mage`.
- `Inference/awac_eval.py`: evaluates categorical AWAC checkpoints on live `Mage`.
- `Offline/awac.py`: trains discrete AWAC from local BC-Minari demonstrations, with optional live `Mage` fine-tuning and TensorBoard logging.
...
- `Custom_enviornments/Test_Env/Mage_BC.py`: no-op, valid-action-only recording environment.
...
- `Custom_enviornments/Test_Env/Mage.py`: current class-specific discrete-action environment.
...
`Custom_enviornments/Test_Env/Mage.py` is the active environment used by all
...
# restore_model_path: runs/Mage__PPO_lstm_server__1__<timestamp>/PPO_lstm_server.pt
...
`mage/BC-v0` ID used by behavior cloning):

```powershell
.\RunOfflineRL.ps1 -Algorithm AWAC -DatasetId "mage/BC-v0"
...
AWAC creates one `Mage` using the recorded BC action order
...
Minari dataset using `MageBC`. Start it together with the WebSocket bridge:
...
overwritten, so use `mage/BC-v1`, `mage/BC-v2`, and so on for later runs.
...
dataset_saved=True reason=truncated id=mage/BC-v0 episodes=1 transitions=256 path=...
```

Inspect a completed dataset and its episode fields with:

```powershell
python -c "import minari; d=minari.load_dataset('mage/BC-v0'); e=next(d.iterate_episodes()); print(d.total_episodes, d.total_steps); print(e.observations.shape, e.actions.shape, e.rewards.shape, e.terminations.shape, e.truncations.shape, e.infos.keys())"
```
...
| `Live` | Yes | Runs the BC model against the live `Mage` game stream. |

Live evaluation translates BC action indices into the different `Mage`
...
Both PPO trainers use one live `Mage` instance directly because the external
...
The recurrent trainer collects one sequential `Mage` rollout and divides it
...
preserved even though `Mage` supports only one live environment. Episode
...
```text
runs/Mage__PPO_lstm_server__<seed>__<timestamp>/PPO_lstm_server.pt
```

The evaluator automatically selects the newest matching run checkpoint, so the
direct `Mage` command is:

```bash
python -m Inference.ppo_lstm_eval
```

It resets the LSTM hidden and cell state at every `Mage` episode boundary. To
...
```text
runs/Mage__PPO_server__<seed>__<timestamp>/PPO_server.pt
```
...
All three evaluators use the direct external `Mage` flow and report episode
...
```powershell
.\RunInference.ps1 -Command "python -m Inference.ppo_lstm_eval --model-path runs/Mage__PPO_lstm_server__1__<timestamp>/PPO_lstm_server.pt"
```
