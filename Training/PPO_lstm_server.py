...
def train(args: Args) -> None:
    ...
    if args.num_envs != 1:
        raise ValueError("Mage supports exactly one live environment")
    ...
    if resume_checkpoint is None:
        run_name = f"Mage__{args.exp_name}__{args.seed}__{int(time.time())}"
    ...
    print(
        "Initializing Mage; waiting for the first game ai_tick...",
        flush=True,
    )
