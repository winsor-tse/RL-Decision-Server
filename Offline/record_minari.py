"""Record live human demonstrations as a Minari MysticBC dataset."""
...
from Custom_enviornments.Test_Env.Mystic_BC import MysticBC
    ...
    if dataset is None:
        with warnings.catch_warnings():
            ...
            dataset = collector.create_dataset(
                ...
                author=author,
                description=(
                    "Human Yugen Saga demonstrations recorded with MysticBC. "
...
def record_minari_dataset(
    *,
    ...
    raw_env: MysticBC | None = None,
):
    """Collect at most ``max_steps`` valid human actions and save a dataset.

    Invalid-key ticks are serviced inside :class:`MysticBC` and never become
    ...
    if destination.exists():
        raise FileExistsError(
            ...
            "Choose a new version, for example mystic/BC-v1."
        )

    base_env = (
        raw_env if raw_env is not None else MysticBC(no_op_action=no_op_action)
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Record valid human Mystic actions into a Minari dataset."
    )
    parser.add_argument(
        "--dataset-id",
