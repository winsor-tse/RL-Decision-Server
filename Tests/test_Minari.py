...
from Custom_enviornments.Test_Env.Mage_BC import (
    BC_ACTIONS_11,
    KEY_TO_ACTION_INDEX,
    MageBC,
    action_from_input,
...
class MageBCMappingTests(unittest.TestCase):
            ...
            with mock.patch.dict(
                ...
                try:
                    ...
                    dataset = collector.create_dataset(
                        ...
                        author="test",
                        description="MageBC integration test",
                    )

                    self.assertEqual(dataset.total_episodes, 1)
                    self.assertEqual(dataset.total_steps, 1)
                    self.assertEqual(dataset.env_spec.id, "YugenSaga/MageBC-v0")
            ...
            with mock.patch.dict(
                ...
                with redirect_stdout(output):
                    dataset = record_minari_dataset(
                        dataset_id="mage/boundary-save-v0",
                        max_steps=2,
                        author="test",
                        raw_env=env,
                    )
    ...
    def test_ctrl_c_flushes_partial_epoch_to_local_minari(self):
        ...
        with tempfile.TemporaryDirectory() as datasets_path:
            with mock.patch.dict(
                ...
                with redirect_stdout(output):
                    dataset = record_minari_dataset(
                        dataset_id="mage/interrupt-save-v0",
                        max_steps=5,
                        author="test",
                        raw_env=env,
                    )
                ...
                reloaded = minari.load_dataset("mage/interrupt-save-v0")
