...
class AWACTests(unittest.TestCase):
    ...
    def test_offline_training_logs_and_saves_without_live_env(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(awac.minari, 'load_dataset', return_value=self.dataset()) as load, mock.patch.object(awac, 'Mage') as env:
                awac.train(config)
            load.assert_called_once_with('mage/BC-v0', download=False)
    ...
    def test_online_uses_bc_order_and_gymnasium_episode_api(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(awac.minari, 'load_dataset', return_value=self.dataset()), mock.patch.object(awac, 'Mage', return_value=env) as factory:
                awac.train(config)
